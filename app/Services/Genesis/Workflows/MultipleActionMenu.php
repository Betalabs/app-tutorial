...
class MultipleActionMenu
{
    const TRANS_PATH = 'app/Services/App/Genesis/Engine/Virtualization/Workflow/Order/MultipleActionMenu.';
    ...
    /**
     * Create Workflow for multiple action menu
     */
    public function create()
    {
        ...
        $identification = WorkflowIdentification::ML_ORDER_MULTIPLE_ACTION_MENU;
        ...
        $appDataParam = $this->searchEventParam('data', $listener->params);
        $idsEventParam = $this->searchEventParam('ids', $event->params);
        $step = (new StepCreator($workflow->id, $listener->id, $approach))
            ->setParams([
                ...
                [
                    'engine_listener_param_id' => $appDataParam->id,
                    'engine_event_param_id' => $idsEventParam->id,
                ],
            ])
            ->create();
